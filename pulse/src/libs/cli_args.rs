...
#[command(
    name = "pulse",
    about = "Network discovery and port scanning daemon",
    version
)]
pub struct Cli {
    #[command(subcommand)]
...
#[derive(Parser, Clone)]
pub struct ScanArgs {
    /// Target hosts/CIDRs (e.g. 192.168.1.0/24, example.com)
    #[arg(short = 't', long, num_args = 1..)]
    pub targets: Vec<String>,

    /// Ports to scan (e.g. "1-1024", "80,443,8080", "top-100", "top-1000")
    #[arg(short = 'p', long, default_value = "top-1000")]
    pub ports: String,
    ...
    /// Delay each concurrent slot waits before its next probe, in milliseconds.
    /// Effective rate: tasks / (delay + probe_time). 0 = no throttling.
    ...
    /// Start fresh (ignore previous scan state)
    #[arg(long, default_value_t = false)]
    pub fresh_start: bool,

    /// TUI event poll timeout in ms
...
pub enum ScanTechnique {
    ...
    Connect,
    /// SYN scan (half-open, requires privileges)
    Syn,
}
...
impl std::fmt::Display for ScanTechnique {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ScanTechnique::Connect => write!(f, "connect"),
            ScanTechnique::Syn => write!(f, "syn"),
        }
    }
}

/// Well-known port lists for convenience.
pub fn resolve_ports(spec: &str) -> Vec<u16> {
    match spec {
        "top-100" => TOP_100_PORTS.to_vec(),
        "top-1000" => TOP_1000_PORTS.to_vec(),
        "all" | "full" => (1..=65535).collect(),
        other => parse_port_spec(other),
    }
}
...
pub const TOP_1000_PORTS: &[u16] = &[
    ...
    1259, 1271, 1272, 1277, 1287, 1296, 1300, 1301, 1309, 1310, 1311, 1322,
    1328, 1334, 1352, 1417, 1433, 1434, 1443, 1455, 1461, 1494, 1500, 1501,
    1503, 1521, 1524, 1533, 1556, 1580, 1583, 1594, 1600, 1641, 1658, 1666,
    1687, 1688, 1700, 1717, 1718, 1719, 1720, 1721, 1723, 1755, 1761, 1782,
    1783, 1801, 1805, 1812, 1839, 1840, 1862, 1863, 1864, 1875, 1900, 1914,
    ...
    2126, 2135, 2144, 2160, 2161, 2170, 2179, 2190, 2191, 2196, 2200, 2222,
    2251, 2260, 2288, 2301, 2323, 2366, 2381, 2382, 2383, 2393, 2394, 2399,
    2401, 2492, 2500, 2522, 2525, 2557, 2601, 2602, 2604, 2605, 2607, 2608,
    2638, 2701, 2702, 2710, 2717, 2718, 2725, 2800, 2809, 2811, 2869, 2875,
    2909, 2910, 2920, 2967, 2968, 2998, 3000, 3001, 3003, 3005, 3006, 3007,
    3011, 3013, 3017, 3030, 3031, 3052, 3071, 3077, 3128, 3168, 3211, 3221,
    3260, 3261, 3268, 3269, 3283, 3300, 3301, 3306, 3322, 3323, 3324, 3325,
    3333, 3351, 3367, 3369, 3370, 3371, 3372, 3389, 3390, 3404, 3476, 3493,
    3517, 3527, 3546, 3551, 3580, 3659, 3689, 3690, 3703, 3737, 3766, 3784,
    3800, 3801, 3809, 3814, 3826, 3827, 3828, 3851, 3869, 3871, 3878, 3880,
    3889, 3905, 3914, 3918, 3920, 3945, 3971, 3986, 3995, 3998, 4000, 4001,
    4002, 4003, 4004, 4005, 4006, 4045, 4111, 4125, 4126, 4129, 4224, 4242,
    4279, 4321, 4343, 4443, 4444, 4445, 4446, 4449, 4550, 4567, 4662, 4848,
    4899, 4900, 4998, 5000, 5001, 5002, 5003, 5004, 5009, 5030, 5033, 5050,
    5051, 5054, 5060, 5061, 5080, 5087, 5100, 5101, 5102, 5120, 5190, 5200,
    5214, 5221, 5222, 5225, 5226, 5269, 5280, 5298, 5357, 5405, 5414, 5431,
    5432, 5440, 5500, 5510, 5544, 5550, 5555, 5560, 5566, 5631, 5633, 5666,
    5678, 5679, 5718, 5730, 5800, 5801, 5802, 5810, 5811, 5815, 5822, 5825,
    5850, 5859, 5862, 5877, 5900, 5901, 5902, 5903, 5904, 5906, 5907, 5910,
    5911, 5915, 5922, 5925, 5950, 5952, 5959, 5960, 5961, 5962, 5963, 5987,
    5988, 5989, 5998, 5999, 6000, 6001, 6002, 6003, 6004, 6005, 6006, 6007,
    ...
    9200, 9207, 9220, 9290, 9415, 9418, 9485, 9500, 9502, 9503, 9535, 9575,
    ...
    14442, 15000, 15002, 15003, 15004, 15660, 15742, 16000, 16001, 16012,
    16016, 16018, 16080, 16113, 16992, 16993, 17877, 17988, 18040, 18101,
    18988, 19101, 19283, 19315, 19350, 19780, 19801, 19842, 20000, 20005,
    20031, 20221, 20222, 20828, 21571, 22939, 23502, 24444, 24800, 25734,
    25735, 26214, 27000, 27352, 27353, 27355, 27356, 27715, 28201, 30000,
