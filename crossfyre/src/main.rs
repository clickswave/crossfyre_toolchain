...
fn workflow_metrics(workflow_id: &str) -> Arc<WorkflowMetrics> {
...
fn workflow_semaphore(workflow_id: &str, tasks: usize) -> Arc<Semaphore> {
...
enum Commands {
    ...
    Login {
        ...
        #[arg(long, default_value = auth::DEFAULT_API_URL)]
        api_url: String,
...
enum NodeAction {
    ...
    /// Run the node supervisor in the foreground: boots every node registered
    /// under `nodes.d` and keeps them online. The OS service execs this; you
    /// normally use `crossfyre node up` instead.
    #[command(hide = true)]
    Supervise {
        /// Force-disconnect already-running instances and take over
        #[arg(long)]
    ...
    /// Run a single registered node as a daemon (normally invoked by the
    /// supervisor).
    Daemon {
...
fn resolve_data_dir(cli_arg: Option<&std::path::Path>) -> Result<std::path::PathBuf, Box<dyn std::error::Error>> {
...
#[cfg(not(unix))]
fn home_for_user(_name: &str) -> Option<std::path::PathBuf> { None }
...
fn chown_tree_to_sudo_user(dir: &std::path::Path) {
...
fn print_privilege_banner(needs_root: bool) {
...
fn nodes_dir(base: &std::path::Path) -> std::path::PathBuf {
    base.join("nodes.d")
}
...
fn migrate_legacy_config(base: &std::path::Path) {
...
async fn async_main() -> Result<(), Box<dyn std::error::Error>> {
    let cli = Cli::parse();
    let base = resolve_data_dir(cli.data_dir.as_deref())?;
    ...
    if requires_login && auth::load_account(&base).is_none() {
    ...
    match cli.command {
        Commands::Login { api_key, username, password, api_url, agree_tos, no_prompt } => {
            auth::run_login(&base, auth::LoginFlags {
                api_key, username, password, api_url, agree_tos, no_prompt,
            }).await?;
        }
        Commands::Logout => {
            auth::run_logout(&base);
        }
        Commands::Node { action } => match action {
            NodeAction::Init { force, api_url, no_service, node_key, no_prompt } => {
                run_init(force, &api_url, &base, no_service, node_key, no_prompt).await?;
            }
            NodeAction::Remove { node_id, inactive } => {
                run_node_remove(&base, node_id, inactive).await?;
            }
            NodeAction::List { json } => {
                run_node_list(&base, json).await?;
            }
            NodeAction::Status => {
                toolchain::status::nodes(&base)?;
            }
            NodeAction::Up { force } => {
                if toolchain::service::node_service_exists() {
                    toolchain::service::start(toolchain::service::NODE_TARGET)?;
                    ...
                    run_boot(force, &base).await?;
                }
            }
            NodeAction::Down => {
                if toolchain::service::node_service_exists() {
                    toolchain::service::stop(toolchain::service::NODE_TARGET)?;
            ...
            NodeAction::Restart => {
                toolchain::service::restart(toolchain::service::NODE_TARGET)?;
            }
            NodeAction::Enable => {
                toolchain::service::enable(toolchain::service::NODE_TARGET)?;
            }
            NodeAction::Disable => {
                toolchain::service::disable(toolchain::service::NODE_TARGET)?;
            }
            NodeAction::Supervise { force } => {
                run_boot(force, &base).await?;
            }
            NodeAction::Daemon { node_id, force } => {
                let paths = NodePaths::new(&base, &node_id);
                run_daemon(force, &paths).await?;
            }
        },
        Commands::Extension { action } => match action {
            ExtensionAction::List => toolchain::status::extensions()?,
            ExtensionAction::Install { name, force } => {
                match name.as_deref() {
                    Some(ext) => {
                        if force {
                            toolchain::install::install(ext, true).await?;
                            for e in toolchain::resolve_extensions(ext)? {
                                toolchain::service::enable(e)?;
                                toolchain::service::start(e)?;
                            }
                        } else {
                            toolchain::install::install_and_start(ext).await?;
                        }
                    }
                    None => toolchain::print_extension_usage("install"),
                }
            }
            ExtensionAction::Remove { name } => {
                match name.as_deref() {
                    Some(ext) => toolchain::install::remove(ext)?,
                    None => toolchain::print_extension_usage("remove"),
                }
            }
            ExtensionAction::Update { name } => {
                toolchain::install::update(name.as_deref()).await?;
            }
            ExtensionAction::Start { name } => toolchain::service::start(&name)?,
            ExtensionAction::Stop { name } => toolchain::service::stop(&name)?,
            ExtensionAction::Restart { name } => toolchain::service::restart(&name)?,
            ExtensionAction::Enable { name } => toolchain::service::enable(&name)?,
            ExtensionAction::Disable { name } => toolchain::service::disable(&name)?,
        },
        Commands::Run { script, targets } => {
            run_script(&script, &targets).await?;
        }
        Commands::Update { target } => {
            let self_updated = toolchain::install::update(target.as_deref()).await?;
            if self_updated && toolchain::service::node_service_exists() {
                println!("Restarting the node service to pick up the new binary...");
                let _ = toolchain::service::restart(toolchain::service::NODE_TARGET);
            }
        }
        Commands::Status => {
            toolchain::status::overview(&base)?;
        }
        Commands::Db { command } => {
            toolchain::db::run(&command)?;
        }
        Commands::Doctor => {
            toolchain::doctor::run(&base).await?;
        }
        Commands::Uninstall { purge } => {
            toolchain::uninstall::run(purge)?;
        }
    }
...
fn spawn_node_daemon(
    ...
    cmd.arg("node").arg("daemon").arg(id);
    if force {
        cmd.arg("--force");
    }
...
async fn run_boot(force: bool, base: &std::path::Path) -> Result<(), Box<dyn std::error::Error>> {
...
fn prompt_node_key(no_prompt: bool) -> Result<String, Box<dyn std::error::Error>> {
    if no_prompt {
...
fn remove_node_files(base: &std::path::Path, node_id: &str) -> std::io::Result<()> {
    let paths = NodePaths::new(base, node_id);
    if paths.config.exists() {
...
fn setup_netns_egress(netns: &str) -> Option<NetnsEgress> {
...
fn print_netns_diagnostics(netns: &str, host_ip: &str) {
...
fn teardown_netns_egress(eg: &NetnsEgress) {
...
fn ensure_tunnel_up(net: &NetworkConfig, net_dir: &std::path::Path, netns: &str) -> (Option<std::process::Child>, Option<NetnsEgress>) {
...
fn wait_for_tunnel(netns: &str, kind: &str, log_path: &std::path::Path) {
...
fn tunnel_in_netns(netns: &str, kind: &str) -> bool {
...
fn print_netns_usage(netns: &str) {
    println!(
...
fn print_tunnel_instructions(net: &NetworkConfig, net_dir: &std::path::Path) {
...
async fn run_daemon(force: bool, paths: &NodePaths) -> Result<(), Box<dyn std::error::Error>> {
