...
pub fn run(purge: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // service (system scope). Each sub-call prints its own per-item lines.
    section("Services");
    ...
    let _ = std::process::Command::new("docker")
        ...
        .output();
    ok("Removed database container (if it existed).");
